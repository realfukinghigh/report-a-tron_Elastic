...
class Tests:
	...
	def createTest(self, engagement_id, test_type, test_exec_summary, test_base_location, testtest_test_limitations, test_main_contact, test_created_on, test_date, test_notes):
		...
		if engagement_id:
			...
			if _engagementStuff.get('asset_id'):
				...
				body = json.dumps({"asset_stuff": _assetStuff, "engagement_stuff": _engagementStuff, "test_stuff": {"engagement_id": engagement_id, "test_type": test_type, "test_exec_summary": test_exec_summary, "test_base_location": test_base_location, "testtest_test_limitations": testtest_test_limitations, "test_main_contact": test_main_contact, "test_created_on": test_created_on, "test_date": test_date, "test_notes": test_notes}})

			else:
				body = json.dumps({"engagement_stuff": _engagementStuff, "test_stuff": {"engagement_id": engagement_id, "test_type": test_type, "test_exec_summary": test_exec_summary, "test_base_location": test_base_location, "testtest_test_limitations": testtest_test_limitations, "test_main_contact": test_main_contact, "test_created_on": test_created_on, "test_date": test_date, "test_notes": test_notes}})

		else:
			body = json.dumps({"test_stuff": {"test_type": test_type, "test_exec_summary": test_exec_summary, "test_base_location": test_base_location, "testtest_test_limitations": testtest_test_limitations, "test_main_contact": test_main_contact, "test_created_on": test_created_on, "test_date": test_date, "test_notes": test_notes}})
	...
	def getTestsForAsset(self, asset_id):
		...
		body = json.dumps({"size":10000, "query": {"bool": {"must": [{"term": {"engagement_stuff.asset_id.keyword": asset_id}},{"exists": {"field": "test_stuff"}}], "must_not": [{"exists": {"field": "issue_stuff"}}]}}})
		sender = self.sess.get(self.url + "_search", headers=self.headers, data=body)
		data = sender.json()['hits']['hits']

		return data

	def updateTest(self, test_id, test_type, test_exec_summary, test_base_location, testtest_test_limitations, test_main_contact, test_date, test_notes):

		body = json.dumps({"doc": {"test_stuff": {"test_type": test_type, "test_exec_summary": test_exec_summary, "test_base_location": test_base_location, "testtest_test_limitations": testtest_test_limitations, "test_main_contact": test_main_contact, "test_date": test_date, "test_notes": test_notes}}})
